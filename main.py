...
def stylize_post(news):
    prompt = f"""
Ты профессиональный автор для Telegram-канала про IT, 3D и нейросети.

Составь интересный, понятный для новичков пост на русском языке по следующему шаблону:

🚀 <b>{news['title']}</b>  
📅 {datetime.now().strftime('%d.%m.%Y')} · {news['source']}

🔹 Краткое объяснение сути в 3-5 предложениях без сложных терминов.  
🔹 Почему это важно или интересно для программистов, 3D-художников или людей в IT.

💬 В конце задай вовлекающий вопрос, чтобы читатели захотели обсудить тему.

<a href="{news['link']}">📖 Читать подробнее</a>

Используй эмодзи, форматирование <b>жирный</b> и <i>курсив</i>.  
Избегай политических тем.  
Пиши живо, без канцелярщины, будто общаешься с другом.
"""
